 ...
 *
 * @devx_uid is the source ucontext's firmware owner-id. CRIU built
 * against an older kernel that doesn't emit it reads zero, which is
 * correct for the v0 critical path (a libibverbs ucontext opened
 * without DEVX). Consumed by the dump path only as image-only
 * metadata (mlx5_vfmig_state_entry.source_devx_uid).
 */
 ...
 * snapshot. DEVX is defined only to document that bit 0 is taken; the v0
 * static path opens without DEVX (non-DEVX source contract).
 */
enum {
