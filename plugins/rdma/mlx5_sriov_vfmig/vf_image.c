 ...
 * vf_image.c
 *
 * On-disk format helpers for the mlx5_sriov_vfmig plugin's
 * per-dump sidecar image. Two artefacts:
 *
 *   1. <image-dir>/mlx5_vfmig.img -- a sequence of length-
 *      prefixed Mlx5VfmigStateEntry protobuf records (one per
 *      restorable uverbs context). Length prefix is a fixed
 *      4-byte little-endian count, no header / no compression.
 *      Hand-rolled framing is used in lieu of the criu/protobuf.c
 *      PB_* helpers because those are tied to criu_image_streamer
 *      and live in the criu binary, not the plugin .so.
 *
 *   2. <image-dir>/<per-VF blob path> -- the raw byte stream
 *      from MLX5_VFMIG_IOC_SAVE_VHCA_STATE's anon-inode fd, one
 *      file per (pf_bdf, vf_id) the plugin captures.
 *
 * The functions here are intentionally kept independent of
 * dump.c / restore.c aggregate types -- they take primitives
 * and raw buffers, not struct vfmig_saved_vf or other plugin
 * state -- so vf_image.c reads as the canonical description
 * of the on-disk shape.
 */
