 ...
 * Shared between the plugin (vfmig_barrier.c) and any peer that speaks
 * the same rendezvous -- i.e. the CRIU instance on the other migration
 * host. Keep this header dependency-free (fixed-width ints only) so both
 * sides agree byte-for-byte and neither drifts.
 *
