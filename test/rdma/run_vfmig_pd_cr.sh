...
# (a unix control socket criu refuses to dump). HOLDER_ALLOC_PD makes the
# holder allocate a PD so the dump captures an R3UT_PD uobject.
...
# source uid is auto-DEVX (uid != 0) for a default libmlx5 ucontext and 0
# only for a rare non-DEVX one; either is fine (uid is dump-side diagnostic
# only), so match any uid.
