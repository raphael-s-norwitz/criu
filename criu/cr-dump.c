...
int cr_dump_tasks(pid_t pid)
{
	 ...
	 * SIGSTOP because we need the pid set the iterator gives us.
	 * The freeze window between collect_pstree()'s SEIZE and this
	 * check is microseconds (one netlink dump + plugin
	 * arbitration), and on failure cr_dump_finish() switches the
	 * tree back to TASK_ALIVE so the snapshot target survives a
	 * coverage rejection. This is just an optimisation over the
	 * dump_uverbsfile() per-fd check that already exists -- it
	 * surfaces the same "no plugin claims this context" error
	 * with the entire tree in view, so the operator gets one
	 * actionable error instead of one per offending fd.
	 */
	 ...
	 * non-snapshot pids that share an exclusive RDMA device with
	 * the snapshot tree. Runs after coverage so we already know
	 * every snapshot-tree context has a claiming plugin to ask.
	 * See criu/rdma.c for the policy + a TOCTOU note.
	 */
