 ...
 *
 * The hard-coded value (119) matches the upstream enum slot the kernel
...
#define RDMA_NLDEV_ATTR_RES_HANDLE 119
#endif
 ...
 * that pair with T1.4. These are enum constants in the kernel header,
 * not preprocessor macros, so use generated compile-test guards rather
 * than #ifndef on the symbolic names. The runtime emission is u32, so
 * this build picks them up by numeric value on an updated kernel and
 * leaves the fields unset on an older one. Drop once the build's
 * minimum rdma-core ships the symbols.
 */
#ifndef CONFIG_HAS_RDMA_NLDEV_ATTR_RES_SEND_CQN
#define RDMA_NLDEV_ATTR_RES_SEND_CQN 120
#endif
#ifndef CONFIG_HAS_RDMA_NLDEV_ATTR_RES_RECV_CQN
#define RDMA_NLDEV_ATTR_RES_RECV_CQN 121
#endif
 ...
 * large enough on either kernel. RES_RECV_CQN (121) is the highest
